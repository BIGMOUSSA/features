...
def to_timestamp(data : pd.DataFrame, date_col : str, time_col:str = None) -> pd.DataFrame :
    ...
    data[date_col] = data[date_col].apply(add_zeros)
    ...
    if time_col != None:
        ...
        data[time_col] = data[time_col].apply(add_zeros)
        data["second"] = pd.to_numeric(data[time_col].str[-2:])
        data["minute"] = pd.to_numeric(data[time_col].str[-4:-2])
        data["hour"] = pd.to_numeric(data[time_col].str[:-4])
